...
def load_model(fname):
    ...
    params = model["params"]
    ttime_spatial = TTime_spatial(params["num_u"], params["dim_u"], params["dict_u"], 
    ...
    ttime_spatial.load_state_dict(model["TTime_gnn"])
    return ttime_spatial

def validate(num_iterations, TTime_spatial):
    total_loss, total_mse, total_l1 = 0.0, 0.0, 0.0
    for _ in range(num_iterations):
        data = test_dataloader.order_emit(args.batch_size)
#         logμ, logλ, mu_c, logvar_c = TTime_spatial(data.trips, data.ratios, data.links.to(device), test_dataloader.get_adj().to(device))
...
for line in f:
    model = line.strip()
    print(model)
    ttime_spatial = load_model(model)
    ttime_spatial.eval()

    tic = time.time()
    with torch.no_grad():
        validate(test_num_iterations, ttime_spatial)
